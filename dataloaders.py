...
def return_dataset(data_name, target_, data_lists,config, mode= 'train', batch_size=16 , test_batchsize = 16, need_balance = True, return_id = False, drop_last = True):
    #print(config)
    datasets = []
    if mode =='train':
    ...
    if data_name in ['vlsc', 'VLSC']:
        construct_dataset = FileListDataset_V
        ...
        construct_dataset = FileListDataset
    ...
    for domain in source_domains:
        ...
        datasets.append(construct_dataset(return_id = return_id, list_path= list_, path_prefix=_prefix,
    ...
    target_dataset = construct_dataset(list_path= target_list, path_prefix=_prefix,transform=trans_t, filter=(lambda x: x in range(num_class)),return_id = return_id )
    print('@@@@@@@@@@ COUNT OF LABELS',Counter (target_dataset.labels))
    target_loader = DataLoader(target_dataset,batch_size=test_batchsize, shuffle=True, drop_last=False,num_workers=8)
